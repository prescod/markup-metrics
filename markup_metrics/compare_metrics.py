...
def calculate_metrics(datadir: Path, engines: List[MetricEngine], prof_log: ProfileLogger, out: Path) -> List[Tuple[str, str, str, float]]:
    ...
    for ref_file in datadir.rglob("*reference.xml"):
        ref_results = process_reference_set(ref_file, ref_file.parent, engines, prof_log, out)
        results.extend(ref_results)
    return results

def process_reference_set(ref_file: Path, datadir: Path, engines: List[MetricEngine], prof_log: ProfileLogger, out: Path) -> List[Tuple[str, str, str, float]]:
...
def main():
    ...
    parser.add_argument(
        "--outdir", type=str, default="./out/test_metrics", help="Path to the output directory."
    )
    ...
    if not metric_engines:
        raise ValueError("No metrics were found. Please check the --metric-engines argument.")

    # Add a check if no data is found
    data_files = list(Path(args.datadir).rglob("*reference.xml"))
    if not data_files:
        raise ValueError(f"No data files were found in the directory {args.datadir}. Please check the --datadir argument.")
